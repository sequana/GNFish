...
def create_directory(directory, path):
    ...
    if not os.path.isdir(directory):
        ...
        print('WARNING. %s directory already exits. New data will be added to '
              'previous one.\n' % directory)


def get_assembly_data(url, label, species, id_num, path,  data_type='genomic',
                      ext='.fna.gz'):
    try:
        ...
        urllib.request.urlretrieve(link, '%s/%s'
                                   % (dir_path, species + '_' + id_num + '_' +
                                      data_type + ext))
        return True
    except:
        dir_path = re.sub('\\\\', '/', path + '/' + directory)
        check_directory(dir_path)
        print('WARNING. No available %s assembly data for %s %s or unable to '
              'download it. Check %s %s on "https://www.ncbi.nlm.nih.gov/" '
              'assembly database.\n'
...
def print_running_info(argument):
    print('INFO. Running --%s argument. Check help or README file for further '
          'information.\n' % argument)
...
def print_successful_download(data_type, species, id_num):
    print('INFO. %s data for %s_%s was successfully downloaded. Check Data/%s '
          'directory.\n' % (data_type.capitalize(), species, id_num,
...
def upddate_rows_info(url, label, species, id_num, rows, exclusive, data_lst, path):
    ...
    for row in rows:
        if row[1] == id_num:
            ...
            row[4] = result[2]
    if not found:
        result = launch_get_assembly_data(url, label, species, id_num,
                                          ['0', '0', '0'], exclusive, data_lst, path)
...
def get_seq_data(term, rows, retmax, db, email, exclusive, data_lst, path):
    ...
    try:
        ...
        print('ERROR. %s from %s not found and can be unpredictable. '
              'Check out "%s" on "https://www.ncbi.nlm.nih.gov/" '
              'assembly database or correct %s terms.\n'
              % (error_sentence, term, term, error_sentence))
    except KeyError:
        ids = record['IdList']
        for id_num in ids:
            summary = get_assembly_summary(id_num, db)
            species = summary['DocumentSummarySet']['DocumentSummary'][0][
                'Organism']
            ...
            if url == '':
                url = summary['DocumentSummarySet']['DocumentSummary'][0][
                    'FtpPath_GenBank']
            if url == '':
                print('WARNING. No available %s data for %s. Check %s in '
...
def check_file(input_file):
    if os.path.isfile(input_file):
        ...
        print("New extraction results will be concatenated at the end of %s "
              "and the already extant ones will be updated.\n" % input_file)
        return True
    else:
        return False


def write_extraction_file(tsv_output_file, refine, retmax, query_file, db,
                          email, exclusive, data_lst, path):
    rows = []
        ...
        for new_row in new_rows:
            for col in new_row:
                if col != '':
                    file.write(str(col) + '\t')
            file.write('\n')
    print('INFO. Genomes download ended. At Data/downloaded_genomes_log.tsv'
          'you can find information about the downloaded genomes.')
...
def main():
    ...
    # Runs the program
    write_extraction_file(path+'/downloaded_genomes_log.tsv', refine, retmax, query_file, db, email, exclusive, data_lst, path)
