...
##Arguments
parser = argparse.ArgumentParser(description='Combines FASTA files in just one matrix')
parser.add_argument('output_name', help='name for the file of the combined matrix', nargs='?', type=str)
parser.add_argument('output_path', help='path were to store the combined matrix.', type=str)
...
parser.add_argument('--pattern', help='By default, the pattern for file searching is “final.fas”. As back up to look for “final_translated.fas”, “RAW.fas”, and “RAW_translated.fas”.', nargs='?', default='final.fas', const='final.fas', type=str)
parser.add_argument('--out_exten', help= 'extension of the output file. Default "_all_combined.fas".', nargs='?', type=str, default='_all_combined.fas')
...
def combine_all_sequences(path, data_type):
    files_lst = get_files(path, pattern)
    with open(output_name + out_exten, "a") as file:
...
if directory is not None:
    for data_type in data_type_lst:
        ...
        combine_all_sequences(directory+'/*', data_type)
else:
    found = False
    for data_type in data_type_lst:
        if data_type is not None:
            ...
            combine_all_sequences(path+'/../Data/'+ data_type.capitalize()+'/*', data_type)
    if not found:
